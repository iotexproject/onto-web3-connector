...
export class NoOnToProviderError extends Error {
  public constructor() {
    super();
    this.name = this.constructor.name;
    this.message = "No OnTo provider was found on window.onto.";
  }
}
...
export class OnToConnector extends AbstractConnector {
  ...
  private handleChainChanged(chainId: string | number): void {
    ...
    this.emitUpdate({ chainId, provider: window.onto });
  }
  ...
  private handleNetworkChanged(networkId: string | number): void {
    ...
    this.emitUpdate({ chainId: networkId, provider: window.onto });
  }

  public async activate(): Promise<ConnectorUpdate> {
    if (!window.onto) {
      throw new NoOnToProviderError();
    }
    ...
    if ((window.onto as any).isMetaMask) {
      (window.onto as any).autoRefreshOnNetworkChange = false;
    }

    // try to activate + get account via eth_requestAccounts
    let account;
    try {
      account = await (window.onto.send as Send)("eth_requestAccounts").then(
    ...
    if (!account) {
      ...
      account = await window.onto
        .enable()
        .then((sendReturn) => sendReturn && parseSendReturn(sendReturn)[0]);
    }

    return { provider: window.onto, ...(account ? { account } : {}) };
  }
  ...
  public async getChainId(): Promise<number | string> {
    ...
    let chainId;
    try {
      chainId = await (window.onto.send as Send)("eth_chainId").then(
        parseSendReturn
      );
    } catch {
      warning(
    ...
    if (!chainId) {
      try {
        chainId = await (window.onto.send as Send)("net_version").then(
          parseSendReturn
        );
      } catch {
        warning(
    ...
    if (!chainId) {
      try {
        chainId = parseSendReturn(
          (window.onto.send as SendOld)({ method: "net_version" })
        );
      } catch {
        warning(
          false,
    ...
    if (!chainId) {
      if ((window.onto as any).isDapper) {
        chainId = parseSendReturn(
          (window.onto as any).cachedResults.net_version
        );
      } else {
        chainId =
  ...
  public async getAccount(): Promise<null | string> {
    ...
    let account;
    try {
      account = await (window.onto.send as Send)("eth_accounts").then(
        (sendReturn) => parseSendReturn(sendReturn)[0]
      );
    } catch {
    ...
    if (!account) {
      account = parseSendReturn(
        (window.onto.send as SendOld)({ method: "eth_accounts" })
      )[0];
    }
  ...
  public async isAuthorized(): Promise<boolean> {
    ...
    try {
      return await (window.onto.send as Send)("eth_accounts").then(
        (sendReturn) => {
